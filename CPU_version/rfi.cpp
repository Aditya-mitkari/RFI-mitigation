...
#if !defined(PRODUCTION_VERSION)
void write_output_file(int nchans, int nsamp, int file_reducer, double orig_mean, double orig_var, float sigma_cut, double var_rescale, double mean_rescale, float *stage)
{
...
void reduce_orig_mean(float *stage, int nsamp, int nchans, double& orig_mean)
{
...
void reduce_orig_var(float *stage, int nsamp, int nchans, double orig_mean, double& orig_var)
{
  for(int iter = 0; iter < nsamp*N; ++iter)
	{
    for( int c = 0; c < nchans; c++ )
...
void channel_process(int *spectra_mask, float *stage, double *chan_mean, double *chan_var, int *chan_mask,
...
void sample_process(int *chan_mask, double *spectra_mean, float *stage, int *spectra_mask,
...
void while_loop1(float *stage, int *chan_mask, double *chan_mean, double *chan_var,
...
void while_loop2(float *stage, int *spectra_mask, double *spectra_mean, double *spectra_var, int nsamp,
...
void transpose_stage_ip(float *stage, unsigned short *input_buffer, int nsamp, int nchans)
{
  for(int t = 0; t < nsamp; ++t)
	{
    for( int c = 0; c < nchans; c++ )
		{
			stage[ c * nsamp + t ] = ( float ) ( input_buffer )[ c  + ( size_t )nchans * t ];
		}
	}
}
...
void rfi_debug(int nsamp, int nchans, unsigned short **input_buffer, double& elapsedTimer)
{
	int file_reducer = 1;
	float	sigma_cut	= 2.0f;
...
int main()
{
  ...
  gettimeofday(&endRFITimer, NULL);
  elapsedRFITimer = (endRFITimer.tv_sec - startRFITimer.tv_sec) + 1e-6 * (endRFITimer.tv_usec - startRFITimer.tv_usec);
#else
